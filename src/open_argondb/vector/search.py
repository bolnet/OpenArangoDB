...
class VectorSearch:
    ...
    def _ensure_embedding_fn(self) -> None:
        ...
        model_name = os.environ.get("ARGONDB_EMBEDDING_MODEL", self._model_name)
    ...
    def _check_native_vector(self) -> bool:
        ...
        try:
            ...
            if not self._db.has_collection("_argondb_vector_check"):
                col = self._db.create_collection("_argondb_vector_check")
                col.add_index({
                    "type": "vector",
                    "fields": ["_test_vec"],
                    "params": {"metric": "cosine", "dimension": 3, "nLists": 1},
                })
                self._db.delete_collection("_argondb_vector_check")
                return True
        except Exception:
            # Clean up on failure
            if self._db.has_collection("_argondb_vector_check"):
                self._db.delete_collection("_argondb_vector_check")
            return False
        return False
